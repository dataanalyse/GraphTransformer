...
def main() -> None:
    ...
    parser.add_argument("--experiment_name", type=str, default="graphormer")
    ...
    X = torch.cat([X, betweenness_feature], dim=2)
    F = X.shape[2]
    ...
    for epoch in range(args.epochs):
        ...
        for t in range(split_t):
            logits = model(X[t], in_degree, out_degree, dist_bucket)
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            with torch.no_grad():
                ...
                for t in range(split_t, T):
                    probs = torch.sigmoid(model(X[t], in_degree, out_degree, dist_bucket))
    ...
    with torch.no_grad():
        ...
        for t in range(T - 5, T):
            probs_last.append(
                torch.sigmoid(model(X[t], in_degree, out_degree, dist_bucket)).squeeze(-1)
            )
