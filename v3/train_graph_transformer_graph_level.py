...
def main() -> None:
    ...
    parser.add_argument("--experiment_name", type=str, default="graph_transformer_graph_level")
    ...
    loss_fn = nn.MSELoss()
    model = GraphTransformerGraphLevel(
        in_dim=F,
    ...
    save_run_start(
        ...
        model=model,
        config={
            "script": "train_graph_transformer_graph_level.py",
            "seed": args.seed,
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            with torch.no_grad():
                ...
                for t in range(split_t, T):
                    preds.append(model(X[t], attn_mask).view(1))
