...
from train_gcn_graph_level import norm_adj
...
class GCNGraphTrajectory(nn.Module):
    def __init__(self, in_dim: int, horizon: int, hid: int = 16):
        super().__init__()
        self.w1 = nn.Linear(in_dim, hid)
        self.w2 = nn.Linear(hid, hid)
        self.head = nn.Linear(hid, horizon)

    def forward(self, x_t: torch.Tensor, a_hat: torch.Tensor) -> torch.Tensor:
        h = a_hat @ x_t
...
def main() -> None:
    ...
    parser.add_argument("--experiment_name", type=str, default="gcn_graph_level_lcc_trajectory")
    ...
    loss_fn = nn.MSELoss()
    model = GCNGraphTrajectory(F, horizon=horizon, hid=args.hidden_dim)
    ...
    save_run_start(
        ...
        model=model,
        config={
            "script": "train_gcn_graph_trajectory.py",
            "seed": args.seed,
