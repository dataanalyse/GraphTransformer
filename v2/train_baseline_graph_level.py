...
def main() -> None:
    ...
    parser.add_argument("--experiment_name", type=str, default="baseline_graph_level")
    ...
    loss_fn = nn.MSELoss()
    model = nn.Sequential(nn.Linear(N * F, 1))
    ...
    save_run_start(
        ...
        model=model,
        config={
            "script": "train_baseline_graph_level.py",
            "seed": args.seed,
    ...
    for epoch in range(args.epochs):
        ...
        loss = loss_fn(pred, Yf[train_idx])
        opt.zero_grad()
        loss.backward()
        opt.step()
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            with torch.no_grad():
                ...
                test_mae = evaluate_mae(test_pred, Yf[test_idx])
